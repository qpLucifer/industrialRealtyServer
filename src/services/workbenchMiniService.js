...
function mergeViewingRows(activeViewings, upcomingViewings) {
  const byId = new Map()
  for (const v of [...activeViewings, ...upcomingViewings]) {
...
/** Ms until viewing matters; null if slot already ended. In-progress → 0. */
function msUntilViewingEvent(view, nowMs) {
  const start = parseViewingSlot(view.start)
  const end = parseViewingSlot(view.end)
  if (!start || !end) return null
  if (end.getTime() <= nowMs) return null
  if (start.getTime() <= nowMs) return 0
  if (start.getTime() > nowMs) return start.getTime() - nowMs
  return null
}

/** Ms until follow-up; null if reminder time already passed. */
function msUntilFollowUp(row, nowMs) {
...
/** Soonest upcoming viewing or follow-up (past times excluded). */
function pickNearestRemindEvent({ viewingRows, followRows, now = new Date() }) {
  ...
  for (const view of viewingRows) {
    const dist = msUntilViewingEvent(view, nowMs)
  ...
  for (const row of followRows) {
    const dist = msUntilFollowUp(row, nowMs)
    if (dist == null) continue
    const at = parseBeijingNaiveToInstant(row.nextReminderAt)
    candidates.push({
      kind: 'follow-up',
...
export async function buildMiniWorkbenchSummary(pool, req) {
  ...
  const followWhere = `list_on_mini = 1 AND next_reminder_at IS NOT NULL AND next_reminder_at > NOW()`
  ...
  const viewingRows = mergeViewingRows(activeViewings, upcomingViewings)
